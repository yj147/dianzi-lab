...
#### TC-F-013A: 点子详情 - 报价与支付状态

- **Requirement**: US-5 点子详情页报价与支付管理
- **Priority**: Medium
- **Preconditions**:
  - 管理员已登录
  - 存在任意点子（任意状态均可）
- **Test Steps**:
  1. 进入点子详情页 `/admin/ideas/[id]`
  2. 在「报价金额（元）」输入框输入合法金额（如 `199.99`），点击「保存」
  3. 在支付状态为「待支付」时点击「确认付款」，在弹窗中点击「确认」
  4. 在支付状态为「已支付」时点击「标记退款」，在弹窗中点击「确认」
- **Expected Results**:
  - 报价保存成功并在刷新后保持一致
  - 支付状态徽标在三种状态间正确更新
  - 支付状态变为「已支付」时显示付款时间；从「已支付」变为「已退款」时不应清空原付款时间
- **Postconditions**: 点子报价/支付状态已更新
...
| US-5 管理后台       | TC-F-011~018, TC-F-013A, TC-E-009, TC-E-010                          | ✓ Complete |
