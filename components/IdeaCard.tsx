...
export default function IdeaCard({ idea, className }: IdeaCardProps) {
  return (
    ...
    >
      ...
      <p className="mb-4 flex-grow text-sm text-muted-foreground line-clamp-3">{idea.description}</p>

      <div className="mt-auto flex items-center justify-between border-t border-border pt-4">
        <div className="flex gap-2">
          {idea.tags.slice(0, 2).map((tag) => (
            <span key={tag} className="rounded-md bg-muted px-2 py-1 font-mono text-xs text-muted-foreground">
              {tag}
            </span>
          ))}
        </div>
