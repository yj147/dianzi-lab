...
export default function PulseDot({ className, paused, style, ...props }: PulseDotProps) {
  ...
  return (
    <span
      ref={dotRef}
      className={cn('inline-block size-2 rounded-full animate-pulse motion-reduce:animate-none', className)}
      style={{
        ...style,
        ...(reducedMotion ? null : { animationPlayState: shouldPause ? 'paused' : 'running' }),
      }}
      aria-hidden="true"
      {...props}
    />
