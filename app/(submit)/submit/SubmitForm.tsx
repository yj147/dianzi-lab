...
export default function SubmitForm() {
    ...
    <div className="mb-8 flex items-center justify-center gap-2">
      {(
        ...
        <div key={s.key} className="flex items-center gap-2">
          {i > 0 ? (
            <div className="h-px w-6 bg-brand-dark/20" aria-hidden="true" />
          ) : null}
          <div
            className={cn(
              ...
              step === s.key
                ? 'border-brand-dark bg-brand-dark text-background dark:border-primary dark:bg-primary dark:text-primary-foreground'
  ...
  if (step === 'idea') {
    const rawPreviewTags = tagsText
      ...
      .filter(Boolean)
    const previewTags =
      rawPreviewTags.length > 0 ? rawPreviewTags : ['标签1', '标签2']
        ...
        <div className="grid items-start gap-8 md:gap-12 lg:grid-cols-2">
          ...
          <div className="sticky top-28 hidden lg:block animate-fade-in-right motion-reduce:animate-none">
            ...
            <div className="rounded-2xl border-2 border-brand-dark/10 bg-muted p-8">
              <IdeaCard
                idea={previewIdea}
                className="pointer-events-none transform scale-105"
              />
              ...
              <div className="mb-6 flex items-start gap-3 rounded-xl border-2 border-red-200 bg-red-50 p-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-pretty text-sm text-red-700/80">
                    当前评分 {result.finalScore} 分，需达到{' '}
                    {MIN_SCORE_TO_SUBMIT} 分才能提交。请优化后重新评估。
                  </p>
                </div>
              </div>
            ) : null}
